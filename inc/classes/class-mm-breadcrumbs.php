...
class Mm_Breadcrumbs {
	 ...
	 */
	public $css = array(
		'module'    => 'mm-breadcrumbs',
		'content'   => 'mm-breadcrumbs-content',
		'wrap'      => 'mm-breadcrumbs-wrap',
		'browse'    => 'mm-breadcrumbs-browse',
		'item'      => 'mm-breadcrumbs-item',
		'separator' => 'mm-breadcrumbs-item-sep',
		'link'      => 'mm-breadcrumbs-item-link',
		'target'    => 'mm-breadcrumbs-item-target',
	);
	...
	function __construct( $core, $args = array() ) {
		...
		$defaults = array(
			...
			'labels'            => array(),
			'date_labels'       => array(),
			// Mm team editing start
			'action'            => 'Mm_breadcrumbs_render',
			'css_namespace'     => array(),
			'path_type'         => 'full',
			// Mm team editing end
			'post_taxonomy'     => apply_filters(
				'Mm_breadcrumbs_trail_taxonomies',
				array(
					'post' => 'category',
				)
			),
		...
		$this->args = apply_filters( 'Mm_breadcrumb_args', wp_parse_args( $args, $defaults ) );
	...
	public function get_trail() {
		...
		$wrapper_classes = apply_filters( 'Mm_breadcrumbs_wrapper_classes', $wrapper_classes );
	...
	public function get_items() {
		...
		$breadcrumb = apply_filters( 'Mm_breadcrumbs_trail', $breadcrumb, $this->args );
	...
	public function get_title() {
		...
		$title = apply_filters(
			'Mm_page_title',
			sprintf( $this->args['page_title_format'], $this->page_title ),
			$this->args
		);
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => esc_html__( 'Browse:', 'mm' ),
			'home'                => $this->home_title(),
			'error_404'           => esc_html__( '404 Not Found', 'mm' ),
			'archives'            => esc_html__( 'Archives', 'mm' ),
			'search'              => esc_html__( 'Search results for &#8220;%s&#8221;', 'mm' ),
			'paged'               => esc_html__( 'Page %s', 'mm' ),
			'archive_minute'      => esc_html__( 'Minute %s', 'mm' ),
			'archive_week'        => esc_html__( 'Week %s', 'mm' ),
	...
	public function home_title() {
		...
		$title            = esc_html__( 'Home', 'mm' );
		$use_custom_title = apply_filters( 'Mm_breadcrumbs_custom_home_title', true );
	...
	public function build_trail() {
		...
		$custom_trail = apply_filters( 'Mm_breadcrumbs_custom_trail', false, $this->args );
		 ...
		 */
		$this->items = apply_filters( 'Mm_breadcrumbs_items', $this->items, $this->args, $this );
	...
	function add_path_parents( $path ) {
		...
		// process default Mm permalinks by own way
	...
	public function prepare_label( $label, $fallback = false ) {
		...
		if ( false !== $fallback ) {
			$fallback = sprintf(
				apply_filters( 'Mm_breadcrumbs_icon_fallback', '<span class="hidden">%s</span>', $fallback ),
				esc_attr( $fallback )
			);
		}
		...
		// Return default icon
		return sprintf(
			apply_filters( 'Mm_breadcrumbs_default_icon_format', '<span class="%1$s">%2$s</span>', $label, $fallback ),
			esc_attr( $label ),
			$fallback
		);
